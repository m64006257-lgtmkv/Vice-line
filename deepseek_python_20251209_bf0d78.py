...
class UnifiedMultiplayerSystem:
    ...
    def _update_local_player(self):
        ...
        if player_data:
            ...
            # إذا كان لدينا اتصال C++، تحديث النواة
            if self.cpp_controller and self.cpp_controller.connected:
                # يمكن إرسال البيانات عبر C++ للشبكة
                pass
    ...
    def _network_loop(self):
        """حلقة الشبكة (للمضيف فقط)"""
        print("🌐 Starting network loop...")
        
        while self.running:
            try:
                # استقبال البيانات من الشبكة
                # إرسال البيانات إلى اللاعبين الآخرين
                time.sleep(0.1)
            except Exception as e:
    ...
    def shutdown(self):
        ...
        # إخراج الـ DLL إذا لزم
        if self.game_pid and self.mode != SystemMode.STANDALONE:
            print("🗑️ Ejecting DLL...")
            # نحتاج إلى معرف handle الـ DLL أولاً
        ...
        try:
            ...
            if self.cpp_controller:
                ...
                for i in range(write_count):
                    self.cpp_controller.write_memory_int(0x00400000, i)
                
                write_time = time.time() - start_time
                results['tests']['cpp_write_speed'] = {
                    'ops': write_count,
                    'time': write_time,
                    'ops_per_sec': write_count / write_time
                }
...
def main():
    ...
    if system.initialize(is_host):
        ...
        print("Check that GTA Vice City is running and DLL is compiled")
